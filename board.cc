...
tile::tile(
    SDL_Rect pos, SDL_Surface* bg, SDL_Surface* focused, Uint8 lx, Uint8 ly) {
    position = pos;
    bg_not_focused = bg;
    bg_focused = focused;
    x = lx;
...
Uint8 tile::get_x() { return x; }

Uint8 tile::get_y() { return y; }
...
bool tile::is_in(Sint32 x, Sint32 y) {
    ...
    if ((x >= position.x) && (x < position.x + position.w) &&
        ...
        if (ly >= (position.h * lx / position.w) + (position.h / 2))
            return false;
        if (ly <= -(position.h * lx / position.w) + (position.h / 2))
            return false;
        if (ly <= (position.h * lx / position.w) - (position.h / 2))
            return false;
        if (ly >= -(position.h * lx / position.w) + 1.5 * position.h)
            return false;
        return true;
    }
    return false;
...
void board::reset() {
    ...
    for (Uint16 i = 0; i < 4; i++) scores[i] = 0;
}
...
board::board() {
    ...
    // init blobs
    for (Uint8 i = 0; i < 8; i++)
        for (Uint8 j = 0; j < 8; j++) blobs[i][j] = NULL;
    ...
    for (Uint8 i = 0; i < 8; i++) {
        for (Uint8 j = 0; j < 8; j++) {
            ...
            if (holes.get(i, j)) {
                ...
                tiles[i * 8 + j] = new tile(r, tilegrass, tilegrasshigh, i, j);
        }
    }
}
        ...
        if (selected_tile != NULL) {
            ...
            if (blobs[fx][fy] != NULL) {
                ...
                if (game->bwrules->authorize_selection(fx, fy)) {
                    unselect_tile(selected_tile->get_x(),
                                  selected_tile->get_y());
            ...
            // else try move
                ...
                game->bwrules->set_move(
                    selected_tile->get_x(), selected_tile->get_y(), fx, fy);
        } else {
            // if not try select
            if (game->bwrules->authorize_selection(fx, fy)) {
                selected_tile = tiles[8 * fx + fy];
    ...
    for (Uint8 i = 0; i < 8; i++) {
        for (Uint8 j = 0; j < 8; j++) {
            if (holes.get(i, j)) {
                ...
                tiles[i * 8 + j]->set_tile(tilegrass, tilegrasshigh);
        }
    }
...
void board::activate() { active = true; }
...
void board::select_tile(Uint8 x, Uint8 y) {
    ...
    for (Sint8 i = -2; i < 3; i++)
        for (Sint8 j = -2; j < 3; j++) {
            ...
            if ((i == 0) && (j == 0)) continue;
            if ((i == 2) || (i == -2) || (j == 2) || (j == -2))
                tiles[8 * (x + i) + y + j]->flip(h2);
            else
                tiles[8 * (x + i) + y + j]->flip(h1);
        }
...
void board::copy_board(bidiarray<bool>& destination) { destination = holes; }
