...
button::button(SDL_Rect pos, string text) {
    ...
    if ((pos.w > 4) && (pos.h > 4)) {
        ...
        SDL_FillRect(
            unfocused, NULL, SDL_MapRGB(focused->format, 106, 91, 255));
...
void button::focus() { surface = focused; }

void button::unfocus() { surface = unfocused; }

void button::setcallback(Uint32 number) { callbacknumber = number; }
