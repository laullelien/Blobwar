...
bool player::is_human() { return (player_type == 0); }

bool player::is_computer() { return (player_type == 1); }
...
rules::rules(Uint16 type, board* b, Uint32 local_player_id) {
    ...
    for (Uint8 i = 0; i < 8; i++)
        for (Uint8 j = 0; j < 8; j++) blobs.set(i, j, -1);
    ...
    if (type == NETGAME) {
#ifdef DEBUG
        cout << "starting network game ; local player is number "
             << local_player_id << endl;
#endif
...
#ifdef DEBUG
    cout << "player " << CURRENT_PLAYER << " selected blob at " << (Uint32)x
         << "," << (Uint32)y << endl;
#endif
    return true;
}
...
bool rules::valid_move() {
    ...
    if ((ox < 0) || (ox > 8) || (nx < 0) || (nx > 8) || (oy < 0) || (oy > 8) ||
        (ny < 0) || (ny > 8)) {
#ifdef DEBUG
...
#ifdef DEBUG
        cout << "trying to move from a blob of the other side, refusing move"
             << endl;
#endif
        return false;
    }
...
bool rules::set_move(Uint8 oldx, Uint8 oldy, Uint8 x, Uint8 y) {
    ...
    if (valid_move()) {
#ifdef DEBUG
        cout << "move from (" << (Uint32)oldx << "," << (Uint32)oldy << ") to ("
             << (Uint32)x << "," << (Uint32)y << ") accepted" << endl;
#endif
        if (gametype == NETGAME) {
...
void rules::do_move() {
#ifdef DEBUG
    cout << "player: " << (CURRENT_PLAYER) << " moving from: " << (Uint32)ox
         << "," << (Uint32)oy << " to " << (Uint32)nx << "," << (Uint32)ny
         << endl;
#endif
    if (!valid_move()) {
        cerr << "invalid move from player " << CURRENT_PLAYER << ", he loses!"
             << endl;
        ...
        for (Sint8 i = -1; i < 2; i++)
            for (Sint8 j = -1; j < 2; j++) {
                ...
                if (ny + j < 0) continue;
                if (ny + j > 7) continue;
                if ((blobs.get(nx + i, ny + j) != -1) &&
                    (blobs.get(nx + i, ny + j) != current_player)) {
                    game->bwboard->change_blob_owner(
                        nx + i, ny + j, current_player);
...
#ifdef DEBUG
    cout << "Timer out. Now kill the IA (pid: " << (*((pid_t*)d)) << ")"
         << endl;
#endif
...
void rules::compute_move() {
    ...
    int status;
    while (wait(&status) != childPid) /* empty */
        ;
...
#ifdef DEBUG
    cout << "computer computed move from: " << (Uint32)ox << "," << (Uint32)oy
...
void rules::next_turn() {
    ...
    for (Uint16 i = 0; i < number_of_players; i++) alive[i] = false;
    for (Uint8 x = 0; x < 8; x++)
        for (Uint8 y = 0; y < 8; y++)
            if (blobs.get(x, y) != -1) alive[blobs.get(x, y)] = true;
    ...
    if (num != 1) {
        ...
        for (Uint16 i = 0; i < number_of_players; i++) can_move[i] = false;
