...
character::character(TTF_Font* font,
                     SDL_Color clrFg,
                     SDL_Color clrBg,
                     char letter) {
...
character::~character() { SDL_FreeSurface(img); }
...
SDL_Surface* font::render_text(string text) {
    ...
    final_img = SDL_CreateRGBSurface(SDL_SWSURFACE,
                                     final_x,
                                     y,
    ...
    //  was 0,254,0
    SDL_SetColorKey(
        final_img, SDL_SRCCOLORKEY, SDL_MapRGB(final_img->format, 0, 0, 0));
