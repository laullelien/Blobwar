 ...
 * NO COPY, don't free the existing surface afterwards, don't register it twice
 * to the engine
 * */
image::image(SDL_Surface* existing_surface) { surface = existing_surface; }

/**free an image
 */
image::~image() { delete surface; }
...
SDL_Surface* image::get_surface() { return surface; }
...
void image::draw_box(SDL_Rect box, SDL_Color color) {
    ...
    switch (surface->format->BitsPerPixel) {
        ...
        default:
            cerr << "warning : direct pixel access not implement for this bpp"
                 << endl;
    }
