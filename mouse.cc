...
Sint16 mouse::get_x() { return mousepos.x; }

Sint16 mouse::get_y() { return mousepos.y; }

void mouse::register_rollover(rollover* r) { rollovers.push_back(r); }

void mouse::unregister_rollover(rollover* r) { rollovers.remove(r); }

/**this method iterates on the list of all widgets affected by mouse position
 * (rollovers) when the mouse is above a rollover it calls rollover::focus and
 * rollover::unfocus else */
void mouse::recompute_focuses() {
    for (list<rollover*>::iterator it = rollovers.begin();
         it != rollovers.end();
         it++) {
...
void mouse::click() {
    for (list<rollover*>::reverse_iterator it = rollovers.rbegin();
         it != rollovers.rend();
         it++) {
