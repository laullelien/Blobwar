...
static int eventfilter(const SDL_Event *event) {
    if (event->type == SDL_KEYDOWN) {
        switch (event->key.keysym.sym) {
            case SDLK_f:
                if (event->key.keysym.mod & KMOD_ALT) game->fullscreen();
                return 0;
            default:
                break;
        }
...
static SDL_Surface *LoadIconSurface(char *file, Uint8 **maskp) {
    ...
    *maskp = NULL;
    ...
    for (i = 0; i < mlen;) {
        if (pixels[i] != *pixels) mask[i / 8] |= 0x01;
        ++i;
        if ((i % 8) != 0) mask[i / 8] <<= 1;
    }
    *maskp = mask;
...
string blobwar::rename(string shortname) { return "data/" + shortname; }
    ...
    while (SDL_PollEvent(&event) != 0) {
        switch (event.type) {
            ...
            case SDL_KEYDOWN:
                ...
                switch (event.key.keysym.sym) {
                    case SDLK_q:
                        if (event.key.keysym.mod & KMOD_CTRL) quit();
                        break;
                    default:
                        break;
                }
...
void blobwar::register_widget(widget *w) { widgets.push_back(w); }

void blobwar::unregister_widget(widget *w) { widgets.remove(w); }
...
void blobwar::set_map_label(string filename) { boardlabel->set_text(filename); }

void blobwar::set_main_label(string text) { mainlabel->set_text(text); }

void blobwar::set_scores_label(string s) { scoreslabel->set_text(s); }
...
void blobwar::start_game(Uint32 local_player_id) {
    ...
    if ((gametype == GAME2P) || (gametype == GAME1P) ||
