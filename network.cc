...
char* network::connect() {
    ...
    if (size <= 0) {
        cerr << "no connection confirmation from server: " << SDL_GetError()
             << endl;
    ...
    if (strncmp(receive_buffer, "/welcome", 8) == 0) {
        ...
        cerr << "error: received msg: \"" << receive_buffer
             << "\" during connection" << endl;
        SDLNet_TCP_Close(sock);
        return NULL;
    }
}
...
void network::handle() {
    ...
    if (SDLNet_SocketReady(sock)) {
        ...
        if (size <= 0) {
            cerr << "error receiving message from server (disconnected ?) "
                 << SDL_GetError() << endl;
            connected = false;
            SDLNet_TCP_Close(sock);
...
void network::execute_messages(Uint32 startposition, Uint32 size) {
    ...
    cout << "executing on :\"" << (receive_buffer + startposition) << "\""
         << endl;
    while (position < size) {
...
Uint32 network::get_player_type() { return playertype; }
